##Input count table
counttable = read.csv("teste_pca.csv", header = TRUE, row.names = 1, sep = ",")
head(counttable)
...
# we get the counts
counts = read.table("teste_pca.csv")
...
library(DESeq2)
ntop = 500
dds = data.matrix(counttable, rownames.force = NA) ##counttable to matrix
